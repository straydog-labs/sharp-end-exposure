...
const intentionSql = readFileSync(join(root, 'sql/training-sessions-intention.sql'), 'utf8');
...
assert.ok(/id="train-mindful-intention"/.test(index));
assert.ok(/What's your focus for this session\? \(optional\)/.test(index));
assert.ok(/id="mindful-enabled-toggle"/.test(index));
assert.ok(/Show pre-workout check-in/.test(index));
assert.ok(/see_mindful_enabled/.test(index));
assert.ok(/function mindfulEnabled\(/.test(index));
assert.ok(/function toggleMindfulEnabled\(/.test(index));
...
assert.ok(/mindfulEnabled\(\)/.test(enter));
assert.ok(/beginTrainSessionAfterMindful\(\)/.test(enter));
...
assert.ok(/train-mindful-intention/.test(finish));
...
assert.ok(/intention:\s*trainSessionState\.intention/.test(save));
assert.ok(/screen-train-done/.test(save));
assert.ok(/add column if not exists intention text/.test(intentionSql));
assert.ok(/training_sessions/.test(intentionSql));
assert.ok(!/notes/.test(intentionSql.split('\n').filter(function(l){ return /add column/.test(l); }).join('\n')));
